...
type AddTagScreenProps = {
  navigation: any;
  // Hand-off from the create row on the event-tag list. The tags were
  // already entered there — and tags are the ONLY thing this setup step
  // still collects (the date and location pickers were removed; both are
  // state-only now) — so there is nothing left to ask and it generates
  // straight away.
  //
  // The founder's "先建立好再設定" on 2026-09-02 was this screen showing
  // its SETUP form, with a spinning generate button, during the seconds
  // the session row was being written. Pressing 建立 and landing on a
  // settings page reads as create-then-configure. autoGenerate now
  // suppresses that form entirely: loader, then the QR.
  route?: { params?: { initialTags?: string[]; autoGenerate?: boolean } };
};
...
export default function AddTagScreen({ navigation, route }: AddTagScreenProps) {
  ...
  // Seeded in the initializer, not an effect: handleGenerateQr reads this
  // state, so the tags must already be there when auto-generation fires.
  ...
  // Fires once. Generation inserts a scan_session row, so a second run
  // would orphan one.
  const autoGeneratedRef = useRef(false);
  useEffect(() => {
    if (!route?.params?.autoGenerate) return;
    if (autoGeneratedRef.current) return;
    if (!user) return; // handleGenerateQr no-ops without a user; wait for it
    autoGeneratedRef.current = true;
    void handleGenerateQr();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [route?.params?.autoGenerate, user]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* On the auto path the setup form must never flash: the user
          already said everything it asks for, on the list. Show a loader
          until the QR exists. */}
      {mode === 'setup' &&
        (route?.params?.autoGenerate ? <PageLoader /> : renderSetupMode())}
