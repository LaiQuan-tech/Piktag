...
type AddTagScreenProps = {
  navigation: any;
  // Optional params from the quick creator on the event-tag list:
  //   initialTags   seed the tag chips
  //   autoGenerate  skip the setup step and produce the QR immediately
  // The QR view's back button returns to setup, so date / location /
  // presets are still one tap away — nothing is lost by skipping ahead.
...
export default function AddTagScreen({ navigation, route }: AddTagScreenProps) {
  ...
  // Quick-create hand-off from the event-tag list: tags were typed there,
  // so go straight to the QR instead of re-showing a setup step the user
  // has already answered. Guarded by a ref because generation creates a
  // scan_session row — running it twice would leave an orphan.
  ...
  useEffect(() => {
    ...
    // handleGenerateQr is re-created every render and is not a dependency
    // we can stabilise without refactoring it; the ref is what actually
