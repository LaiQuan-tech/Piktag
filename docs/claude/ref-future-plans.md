...
3. **批次標籤 — 贊成,併入 Pro。** 綜效:批次標籤豐富付費者資料 → 其媒合
   與搜回更準 → 貴人王更有感。BatchTagScreen 仍是唯一共用 UI。
   **界線 2026-09-09 改切(創辦人裁決)**:免費/付費不再切在「誰選人」,
   改切在「**怎麼選人**」—— **免費 = 使用者手選**(好友列表選取模式);
   付費 = 使用者自訂條件選人(依標籤/地點/時間/未標籤者)、一次套多標籤、
   整批 AI 建議、匯出選取批次。系統自動提出的批次(爆發偵測、匯入分類)
   一律免費,從來不是界線爭點,不必逐條列;系統的時間 cohort 不因為付費層
   賣「依時間選人」就變付費,差別在**誰發起**。(原記載「付費 = 任選好友」,2026-09-09 探測發現
   手選批次早已免費出貨於 ConnectionsScreen,且 MONETIZATION_ROADMAP
   原則 3「免費層永遠完整可用」禁止把它收回去賣。)
4. **個人標籤置頂 — 修正後贊成,併入 Pro(2026-07-06 二次評估推翻初評)。**
   探測證實:他人檔案頁(FriendDetail)標籤排序是動態的(pick 會上浮),
   且 is_pinned 已是排序鏈最高優先、置頂 UI 早已被刻意移除「保留為未來
   付費功能」(EditProfileScreen.tsx:66-73 註解)。價值主張乾淨:
   **免費 = 社群共識排序(picks 上浮);付費 = 我說了算(置頂鎖第一格)**。
   只影響自己檔案頁、零排序污染,與人氣王本質不同。實作 = 補一個 pin
   開關 UI + 寫 is_pinned,極小。賣車/賣房更完整的需求仍導入
   business_profiles(#23)。初評「單獨不成立」基於過時文件,已修正。
...
**打包與時機(裁決):** 單一 **PikTag Pro $5-8/mo**(貴人王+批次+
標籤置頂+CRM 進階),不做兩個並列 $5 訂閱;真人認證獨立低價;主辦方
獨立 B2B;人氣王需求導入贊助軌道。iOS 訂閱一律走 IAP(含 Apple 抽成定價)。
