...
本檔規則**只適用主對話**。你若是被 Agent 工具派出的 subagent
(prompt 開頭通常是「實作:」「在 X 找出」「你是驗收者」等任務書),
**你就是實作者:親自讀檔、親自改檔、親自驗,禁止再派 subagent**。
真實事故(2026-07-06):三層 sonnet agent 各自讀到本規則,層層轉派、
互相等待、零產出、燒 130k token。判斷法:看得到使用者的對話歷史 =
主對話;只看得到一份任務書 = subagent。
