...
def _linux_fallback(size: int, bold: bool) -> ImageFont.FreeTypeFont:
    # macOS system fonts don't exist on Linux (CI / container). Map each
    # script to an installed Noto/Liberation face so every locale renders
    # (raqm handles Arabic/Devanagari/Thai shaping). Purely a fallback —
    # the macOS paths above stay the authored look on the founder's Mac.
    w = "Bold" if bold else "Regular"
    if LANG in ("zh-TW", "zh-CN", "ja", "ko"):
        # Noto CJK covers Latin too (so embedded "QR"/"PikTag" render).
        return ImageFont.truetype(
            "/usr/share/fonts/opentype/noto/NotoSansCJK-%s.ttc" % w, size, index=0)
    # ar/hi/th captions embed Latin tokens ("QR", "AI"), so the font must
    # cover BOTH the script AND Latin — the script-only Noto faces tofu the
    # Latin. These families carry both:
    if LANG == "ar":
        return ImageFont.truetype(
